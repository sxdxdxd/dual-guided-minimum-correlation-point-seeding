function ExpressionData = calculateExpressionLevelForStreamLines(X, V, ScalarField, PointPassedByStreamLines, Options)
if ndims(X) == 4
    x = X(:, :, :, 1); y = X(:, :, :, 2); z = X(:, :, :, 3);
    u = V(:, :, :, 1); v = V(:, :, :, 2); w = V(:, :, :, 3);
...
if ndims(X) == 4
    ...
    if Options.ScalarComponent
        [U, V, W, S] = reconstructVectorAndScalarField3D(Mask, u_g, v_g, w_g, ScalarField_g, ...
            Options.OvercorrectionFactor, Options.Mu, Options.DiffusionThreshold, Options.OptimizeMaxIterations);
        ThetaDifferenceField = calculateThetaDifference(cat(4, u, v, w, ScalarField), cat(4, U, V, W, S));
    else
    ...
    N_Bins = 360;
    BinField = calculateBinField_3D(u, v, w, BinMap);
    BinField_R = calculateBinField_3D(U, V, W, BinMap);
    ...
    if Options.ScalarComponent
        [U, V, S] = reconstructVectorAndScalarField2D(Mask, u_g, v_g, ScalarField_g, ...
            Options.OvercorrectionFactor, Options.Mu, Options.DiffusionThreshold, Options.OptimizeMaxIterations);
        ThetaDifferenceField = calculateThetaDifference(cat(3, u, v, ScalarField),cat(3, U, V, S));
    else
    ...
    TotalEntropy = calculateTotalEntropy(BinField, N_Bins);
    ExpressionData.ConditionalEntropy = calculateGlobalConditionalEntropy_mex(BinField, BinField_R, N_Bins, TotalEntropy, 'H(X|Y)');
end
...
ExpressionData.ExpressionLevel = calculateExpressionLevel(Options.alpha, ThetaDifferenceField, OmegaField, ScalarField_R, ScalarField, 'AddConst');
end
